...
def _direct_ollama_call_with_tools(messages, model, tools=None, temperature=0.1, timeout=None, **kwargs):
    """
    ...
    try:
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                print(f"⏰ Ollama call timed out after {timeout}s (attempt {attempt + 1}/{max_retries + 1})")
                if attempt < max_retries:
                    ...
                    return AIMessage(content=f"Request timed out after {timeout}s. The model may be busy or the request too complex. Please try again with a simpler query.")
                ...
                try:
                    response = requests.post('http://localhost:11434/api/chat', json=minimal_payload, timeout=timeout)
                    break  # Success with minimal payload
                except requests.exceptions.Timeout:
                    print(f"⏰ Minimal payload also timed out (attempt {attempt + 1}/{max_retries + 1})")
                    if attempt >= max_retries:
                        return AIMessage(content="Request failed due to timeout and JSON encoding issues. Please try again.")
                    continue
                    
            except requests.exceptions.ConnectionError as e:
                print(f"🔌 Connection error to Ollama: {e}")
                if attempt >= max_retries:
                    return AIMessage(content="Could not connect to Ollama. Please ensure Ollama is running on localhost:11434.")
